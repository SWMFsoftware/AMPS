...
int Moon::Sampling::SubsolarLimbColumnIntegrals::nSampleAltitudeDistributionPoints=0;
...
//print into a file column integrals that correspond to observations of Kaguya's TVIS instrument
 void Moon::Sampling::Kaguya::TVIS::OutputModelData(int DataOutputFileNumber) {
#if _EXOSPHERE__ORBIT_CALCUALTION__MODE_ == _PIC_MODE_ON_
  int i;
  FILE *fout;
    ...
    for (i=0;i<TvisOrientationVector[nDataSet].nTotalTvisOrientationElements;i++) {
      ...
      if (PIC::ThisThread==0) {
        ...
        fprintf(fout,"%e  %e  %e  %e  %e\n",TvisOrientationVector[nDataSet].TvisOrientation[i].EquatorialCrossingTime,TvisOrientationVector[nDataSet].TvisOrientation[i].Epoch,
            IntegralsVector[Moon::Sampling::SubsolarLimbColumnIntegrals::_NA_COLUMN_DENSITY_OFFSET_],IntegralsVector[Moon::Sampling::SubsolarLimbColumnIntegrals::_NA_EMISSION_5891_58A_SAMPLE_OFFSET_],IntegralsVector[Moon::Sampling::SubsolarLimbColumnIntegrals::_NA_EMISSION_5897_56A_SAMPLE_OFFSET_]);
      }
    }
