...
int main(int argc,char **argv) {
  ...
  cudaMemcpy(deviceTreeDescriptorTable,hostTreeDescriptorTable,TreeDescriptorLength*sizeof(cSplitTable),cudaMemcpyHostToDevice); 

  cudaMalloc(&device_xmin,3*sizeof(double));
  cudaMemcpy(device_xmin,PIC::Mesh::CPU::mesh->xGlobalMin,3*sizeof(double),cudaMemcpyHostToDevice);

  cudaMalloc(&device_xmax,3*sizeof(double));
  cudaMemcpy(device_xmax,PIC::Mesh::CPU::mesh->xGlobalMax,3*sizeof(double),cudaMemcpyHostToDevice);

  auto BuildMeshDevice = [=] _TARGET_DEVICE_ (cSplitTable *dTreeDescriptorTable,int dTreeDescriptorLength,double *xmin,double *xmax)  {
    PIC::Mesh::GPU::mesh->BuildTreeWithDescriptor(xmin,xmax,dTreeDescriptorTable,TreeDescriptorLength); 
  };

  kernel_4<<<1,1>>>(BuildMeshDevice,deviceTreeDescriptorTable,TreeDescriptorLength,device_xmin,device_xmax); 
  cudaDeviceSynchronize();


  delete [] hostTreeDescriptorTable;

  cudaFree(deviceTreeDescriptorTable);
  cudaFree(device_xmin);
