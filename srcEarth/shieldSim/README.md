...
The package includes automated test scripts under `tests/`.  Layer-1 tests check
build/CLI behavior.  Layer-2 tests check geometry, source sampling, and shield
rear-face scoring using short deterministic runs.

New diagnostic CLI options used by the Layer-2 tests are:
...
The local coordinates and directions in the exit diagnostic are in the shield
coordinate frame.  They are intended to verify that rear-face scoring uses the
correct local-coordinate boundary condition.

Run the tests from the top-level package directory:

```bash
chmod +x tests/run_layer1_tests.sh tests/run_layer2_tests.sh
tests/run_layer1_tests.sh
tests/run_layer2_tests.sh
```

The Layer-2 script accepts the same `-h`, `-help`, and `--help` options as the
Layer-1 script:
