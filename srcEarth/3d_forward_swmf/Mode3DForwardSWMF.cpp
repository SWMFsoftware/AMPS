...
// Last cutoff-output suffix generated by amps_cutoff_time_step().
//
// The cutoff solver itself receives this suffix through
// Earth::Mode3D::SetCutoffOutputFileSuffix(), while other SWMF-coupled
// diagnostic products, such as the AMPS mesh dump written in main_lib.cpp,
// need to use exactly the same suffix after the cutoff call returns.  Keeping
// the suffix here makes the cutoff product and its companion diagnostics form
// an unambiguous matched set of files:
//
//   cutoff_3d_shells<suffix>.dat
//   amps_coupled_data<suffix>.dat
//
// The value is updated before each coupled cutoff calculation starts.  It is
// intentionally not recomputed by the diagnostic-output code because that would
// duplicate the naming policy and could become inconsistent when the call/time
// stamp format is changed later.
...
void amps_pre_init() {
  ...
  // Reset the cached companion-output suffix during initialization.  The first
  // valid value is produced by amps_cutoff_time_step(), after the first live
  // SWMF field snapshot has been received and just before the cutoff files are
  // written.  Clearing it here prevents a stale suffix from a previous runtime
  // initialization path from being reused accidentally if initialization is ever
  // re-entered in a test harness.
  s_last_cutoff_output_suffix.clear();
...
void GetLastCutoffOutputStamp(char* stamp,int stampLength) {
  // Copy out the suffix that was generated by the most recent
  // amps_cutoff_time_step() call.
  //
  // This routine is deliberately a small accessor rather than a second formatter:
  // the cutoff products and the companion AMPS mesh dump must use the same string
  // byte-for-byte.  If the filename convention changes in FormatCutoffOutputSuffix_(),
  // the companion products follow automatically because they read the cached suffix
  // instead of trying to recreate it.
  //
  // The returned string includes the leading separator, for example
  //
  //   .swmf_n000003_t000600.000s
  //
  // so callers can build names as
  //
  //   <stem><stamp>.dat
  //
  // without knowing any details of the stamp format.
  if (stamp==NULL || stampLength<=0) return;

  // Always leave the caller with a valid C string, even if this function exits
  // early.  This avoids undefined behavior in diagnostic/error paths that may
  // print the buffer contents after a failed call.
  stamp[0]='\0';

#if _PIC_COUPLER_MODE_ != _PIC_COUPLER_MODE__SWMF_
  // Non-SWMF builds never generate coupled cutoff snapshots.  Return an empty
  // suffix so this function remains link-safe when the header is included by
  // generic Earth sources.
  return;
#else
  if (s_last_cutoff_output_suffix.empty()) {
    // main_lib.cpp calls this only after amps_cutoff_time_step() in the cutoff
    // branch.  An empty suffix here therefore means the call order was violated
    // or no cutoff snapshot was actually produced.  Stop explicitly instead of
    // silently writing an unstamped file that could overwrite another snapshot.
    exit(__LINE__,__FILE__,
         "[Mode3DForwardSWMF] GetLastCutoffOutputStamp() called before a cutoff output suffix was generated.");
  }

  const int n = std::snprintf(stamp,static_cast<size_t>(stampLength),"%s",
                              s_last_cutoff_output_suffix.c_str());

  if (n<0 || n>=stampLength) {
    // Truncation would make the companion file name differ from the cutoff file
    // name and could also make different snapshots collide.  Treat this as a
    // configuration/programming error and ask the caller to increase the buffer.
    exit(__LINE__,__FILE__,
         "[Mode3DForwardSWMF] output-stamp buffer is too small in GetLastCutoffOutputStamp().");
  }
#endif
}
...
void GetLastCutoffOutputFileName(char* fileName,
  ...
  // Construct a companion output filename with exactly the same suffix used by
  // the most recent cutoff-rigidity product.
  //
  // This helper centralizes the filename assembly for non-cutoff products that
  // are written immediately after amps_cutoff_time_step().  The intended use is
  //
  //   GetLastCutoffOutputFileName(fname,sizeof(fname),
  //                               "amps_coupled_data",".dat");
  //
  // which produces a name such as
  //
  //   amps_coupled_data.swmf_n000003_t000600.000s.dat
  //
  // matching the cutoff file produced during the same coupling call, for example
  //
  //   cutoff_3d_shells.swmf_n000003_t000600.000s.dat
  //
  // Keeping this logic here avoids duplicating suffix details in main_lib.cpp and
  // makes any future change to the cutoff naming convention local to this module.
  if (fileName==NULL || fileNameLength<=0) return;

  // Leave the output buffer in a valid state before doing any validation.  This
  // is useful if an error message prints the buffer or if a defensive caller
  // checks it after a no-op non-SWMF build.
  fileName[0]='\0';

  if (stem==NULL || stem[0]=='\0') {
    exit(__LINE__,__FILE__,
         "[Mode3DForwardSWMF] GetLastCutoffOutputFileName() requires a non-empty file stem.");
  }

  if (extension==NULL) extension="";

  char stamp[128];
  GetLastCutoffOutputStamp(stamp,static_cast<int>(sizeof(stamp)));

  const int n = std::snprintf(fileName,static_cast<size_t>(fileNameLength),
                              "%s%s%s",stem,stamp,extension);

  if (n<0 || n>=fileNameLength) {
    // A truncated file name could overwrite the wrong product or break the
    // one-to-one correspondence between cutoff files and the companion AMPS mesh
    // dump.  Fail loudly so the fixed-size buffer at the call site can be enlarged.
    exit(__LINE__,__FILE__,
         "[Mode3DForwardSWMF] filename buffer is too small in GetLastCutoffOutputFileName().");
  }
}
...
void amps_cutoff_time_step() {
  ...
  // Cache the exact suffix before any output is written.  The cutoff solver uses
  // the same string through SetCutoffOutputFileSuffix(), while main_lib.cpp reads
  // this cached value immediately after amps_cutoff_time_step() returns to name
  // the companion AMPS mesh dump.  Caching avoids recomputing the stamp after the
  // call counter and elapsed time are advanced below.
