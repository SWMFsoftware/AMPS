...
backtraced with the Boris mover. RK4 is used at selected quiet, compression,
...
Two spatial products are calculated. Observation-equivalent products reproduce
the exact C9 and C10 shells, vertical incidence, rigidity samples, time
aggregation, and $T_{50}$ operators described above. The research morphology
product uses both 475- and 850-km shells, a $15^{\circ}$ longitude by
$2^{\circ}$ latitude grid from $45^{\circ}$ to $85^{\circ}$ in both
...
\end{equation}
\end{linenomath*}
The fit uses the cell uncertainties and resolved fractions. A second harmonic
is retained only if it improves blocked cross-validation and its amplitude is
stable under grid refinement. The storm perturbation is referenced to the
quiet median,
\begin{linenomath*}
\begin{equation}
...
The surface area on a shell that is accessible to rigidity $R$ at a selected
transmission threshold $T_*$ is
\begin{linenomath*}
\begin{equation}
 S_{\mathrm{acc}}(R,t;T_*)=
 \int_{S_h} H[T_{\mathrm{mod}}(R,\mathbf{x},t)-T_*] \, \mathrm{d}S,
 \qquad
 f_{\mathrm{acc}}=S_{\mathrm{acc}}/S_h,
 \label{eq:exp-accessible-area}
\end{equation}
\end{linenomath*}
where $S_h$ is the analyzed area of the altitude shell and $H$ is the Heaviside
function. Results are reported for $T_*=0.5$, with 0.25 and 0.75 as sensitivity
thresholds. Reporting both latitude and accessible area distinguishes a nearly
uniform equatorward shift from a strongly asymmetric opening confined to a
limited range of MLT.
