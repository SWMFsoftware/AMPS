...
for target in test-cli-unit test-state-unit test-geometry-source-unit \
  ...
  test-reproducibility-unit test-wp11-wp20-unit \
...
for source in sep_test_registry sep_transport_common sep_focused_transport_core \
  sep_focused_transport_mfp_core sep_coefficient_physics sep_species_source \
  sep_coefficient_registry \
  sep_background_snapshot sep_acceptance_cases sep_production_mover sep_cli; do
  definitions=
  case "$source" in
    sep_cli) definitions=-DSEP_CLI_PARSE_ONLY ;;
  esac
  ${CXX:-c++} -std=c++11 -Wall -Wextra -Wpedantic -Werror $definitions \
    -I"$src_root/util" -fsyntax-only "$src_root/util/$source.cpp"
done
...
${CXX:-c++} -std=c++11 -Wall -Wextra -Wpedantic -Werror \
  -I"$src_root" -I"$src_root/util" -fsyntax-only \
