...
This case implements validation-plan item **VP09** in the extensible case-owned campaign layout. The numerical target is analytical. It validates production physics and the comparison path; any observational-release limitation is recorded in the result JSON.
...
Sheath/ejecta classifications and region-boundary timing is a distinct validation contract: a passing lower-level unit test does not by itself prove that the case selection, units, independent calculation, acceptance threshold, plotting, and campaign reporting remain mutually consistent.
...
python3 run_vp09.py
# or from swcme/test:
make validation-case CASE=VP09
```
...
The case passes only when every criterion in `vp09_result.json` is true. A PASS is scoped to the evidence class recorded there; limitations are machine-readable and must be carried into any science-release claim.
