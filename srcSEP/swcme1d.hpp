...
// swcme1d.hpp
// ----------------------------------------------------------------------------
// FAST 1-D SOLAR WIND + CME FORWARD-SHOCK MODEL (SANITIZED API)
// ----------------------------------------------------------------------------
// WHAT THIS PROVIDES
// • Semi-analytical 1-D model of a CME-driven shock propagating radially, with:
//    – Upstream solar wind density n(r) via Leblanc–Dulk–Bougeret (1998), scaled
//      to a user-chosen n(1 AU), converted to SI.
//    – Parker spiral magnetic field magnitude and components (Parker 1958).
//    – Apex kinematics from the Drag-Based Model (DBM; Vršnak+ 2013):
//         u(t) = u0 / (1 + Γ u0 t),  r(t) = r0 + Vsw t + ln(1+Γ u0 t)/Γ,
//      where u=Vsh−Vsw, Γ is the drag parameter.
//    – Region structure along the radius at any time t:
//         upstream → [shock] → sheath → [leading edge] → magnetic ejecta
//         → [trailing edge] → downstream ambient
//      with independent C¹ smoothstep blends at shock/LE/TE edges.
//    – Oblique MHD shock proxy (collapsed to quasi-radial 1-D form) to estimate
//      density compression ratio rc and post-shock flow speed at the shock.
//    – Magnetic jump proxy: B_n continuous, B_t amplified ~ rc in the sheath;
//      in 1-D we report Parker Br, Bφ and |B|; tangential amplification is
//      represented implicitly through rc diagnostics and region mapping.
//    – Divergence of the bulk flow ∇·V via robust centered FD of (1/r²) d(r²V)/dr.
// • High-throughput evaluation on an array of radii r[i] (meters).
// • NaN/Inf SANITIZATION in the API: every value written to user arrays is
//   checked and replaced with a physically sensible fallback (see notes below).
// • A Tecplot POINT writer for radial profiles (sanitized).
//
// UNITS
//   r [m], t [s];  n [m⁻³];  V [m/s];  B [Tesla];  divV [s⁻¹].
//
// REFERENCES (titles)
//   • Parker (1958) “Dynamics of the Interplanetary Gas and Magnetic Fields,” ApJ.
//   • Leblanc, Dulk & Bougeret (1998) “Tracing the Electron Density from the
//     Corona to 1 AU,” Solar Physics.
//   • Vršnak & Žic (2007) A&A; Vršnak et al. (2013) Solar Physics — DBM.
//   • Edmiston & Kennel (1984) J. Plasma Phys.; Priest (2014) CUP — MHD shock.
//
// NUMERICAL / EFFICIENCY NOTES
//   • StepState caches Leblanc coefficients (already scaled to match n(1 AU)),
//     Parker normalization at 1 AU, sound speed, and DBM state (r_sh, V_sh).
//   • Hot loops avoid std::pow: use r^-2, r^-4, r^-6 with fused multiplies.
//   • Smoothsteps precompute 1/(2w); branches are simple and predictable.
//   • All evaluators are allocation-free and re-entrant.
//   • API SANITIZATION: every scalar written to n, V, Br, Bφ, |B|, divV is
//     passed through a finite-check with fallbacks:
//       n   → upstream n_up(r) (and clamped ≥ 0)
//       V   → upstream V_sw
//       Br, Bφ, |B| → 0 as last resort (formulas already finite)
//       divV → 0 (neutral fallback)
//     This guarantees NaN/Inf-free arrays even in edge cases.
//
// BUILD EXAMPLE
...
// ------------------------------
// Runtime I/O toggle (optional helpers)
// ------------------------------
...
// ------------------------------
// Numeric safety helpers (used for API sanitation)
// ------------------------------
...
struct Params {
  ...
  // Parker pitch (≈1 in ecliptic); we treat sinθ as constant along the ray
  ...
  // Independent C¹ smooth widths at (shock, LE, TE), scaled by R_sh
...
struct StepState {
  ...
  double rc      = 1.0; // density compression ratio at shock (≥1)
  ...
  // Cached Parker/Leblanc constants for this tick
  double lebA=0, lebB=0, lebC=0;  // Leblanc cm^-3 coefficients scaled to n(1 AU)
...
public:
  ...
  // Build caches for time t: DBM R_sh,V_sh; rc via fast Mach; regions & widths; Parker/Leblanc constants.
  StepState prepare_step(double t_s) const {
    StepState S{};
    S.t_s=t_s;
    S.V_up_ms = V_sw();

    // --- DBM apex kinematics (analytic) ---
    double r_sh, V_sh; shock_position_speed(t_s, r_sh, V_sh);
    S.r_sh_m  = r_sh; S.V_sh_ms = V_sh;

    // --- Leblanc constants scaled to match n(1 AU) ---
    const double nLeb1AU = leblanc_cm3(AU);
    const double scale = (nLeb1AU>0.0)? (P.n1AU_cm3 / nLeb1AU) : 1.0;
    S.lebA = scale * 3.3e5; S.lebB = scale * 4.1e6; S.lebC = scale * 8.0e7;
    ...
    if (cf<=0.0 || Vrel<=0.0) {
      ...
      // one-fluid continuity proxy for downstream speed at the shock:
      S.V_dn_ms = S.V_sh_ms + (S.V_up_ms - S.V_sh_ms)/S.rc;
    }

    // --- Self-similar region geometry ---
    const double scaleR = S.r_sh_m / AU;
    S.dr_sheath_m = std::max(0.0,P.sheath_thick_AU_at1AU) * scaleR * AU;
    S.dr_me_m     = std::max(0.0,P.ejecta_thick_AU_at1AU) * scaleR * AU;
    S.r_le_m      = S.r_sh_m - S.dr_sheath_m;
    S.r_te_m      = S.r_le_m - S.dr_me_m;
    S.inv_dr_sheath = (S.dr_sheath_m>0.0)? 1.0/S.dr_sheath_m : 0.0;

    // --- Independent C¹ smooth widths ---
    S.w_shock_m = std::max(0.0,P.edge_smooth_shock_AU_at1AU) * scaleR * AU;
    S.w_le_m    = std::max(0.0,P.edge_smooth_le_AU_at1AU)    * scaleR * AU;
    S.w_te_m    = std::max(0.0,P.edge_smooth_te_AU_at1AU)    * scaleR * AU;
                           ...
                           std::size_t N) const
  {
    ...
    for (std::size_t i=0;i<N;++i){
      ...
      // Upstream density: scaled Leblanc [cm^-3] → [m^-3]
      const double s = Rs/r;
      const double inv2=s*s, inv4=inv2*inv2, inv6=inv4*inv2;
      const double n_up = (S.lebA*inv2 + S.lebB*inv4 + S.lebC*inv6) * 1e6; // [m^-3]
      ...
      const double Csheath = rc_floor + (rc - rc_floor)*ramp;     // ≥ rc_floor
      ...
      // ---- SANITIZE outputs written to user arrays ----
      n_m3[i] = sw1d_finite_or(n_mix, n_up);
      V_ms[i] = sw1d_finite_or(V_mix, Vup);
      if (n_m3[i] < 0) n_m3[i] = 0; // guard against round-off
    }
  }
                                 ...
                                 double dr_frac=1e-3) const
  {
    ...
    // Parker spiral components — sanitize each component
    for (std::size_t i=0;i<N;++i){
      const double r    = std::max(r_m[i], 1.05*Rs);
    ...
    // Divergence: robust centered difference + sanitization
  ...
  // Convenience: build the arrays internally and write Tecplot line.
...
private:
  ...
  // Leblanc baseline (cm^-3) before scaling
  ...
  // Parker magnitude at r from S (uses cached Br1AU_T, k_AU)
  ...
  // Alfven speed using scaled Leblanc density
  static inline double v_A(const StepState& S, double r_m){
    const double s=Rs/r_m; const double s2=s*s, s4=s2*s2, s6=s4*s2;
    const double n_up = (S.lebA*s2 + S.lebB*s4 + S.lebC*s6) * 1e6; // m^-3
  ...
  // Sample only V(r) using exactly the same blending as evaluate_radii_fast (sanitized)
