...
const int nTotalCell= _BLOCK_CELLS_X_*_BLOCK_CELLS_Y_*_BLOCK_CELLS_Z_; 

#pragma omp parallel for schedule(dynamic,nTotalCell) default (none) firstprivate (ParticleDataListLength,SigmaCrMax_nTest,SigmaCrMax_SafetyMargin) \
...
//Majorant Frequency collision procedure
void PIC::MolecularCollisions::ParticleCollisionModel::mf_new() {
  int SigmaCrMax_nTest=100;
  double SigmaCrMax_SafetyMargin=1.3;
...
//                  if (s0==s1) ancoll=0.5*sumWeightCorrection_s0*(nParticleNumber[s0]-1.0)*LocalParticleWeight_s0*SigmaCrMax*LocalTimeStep_s0/cellMeasure;
		  ...
		  /*
		  double meanWeightCorr[2];
		  meanWeightCorr[0]=sumWeightCorrection_s0/nParticleNumber[s0];
		  meanWeightCorr[1]=sumWeightCorrection_s1/nParticleNumber[s1];
		  */
			...
			*((double*)(SamplingData+CollFreqOffset))+=PIC::ParticleBuffer::GetIndividualStatWeightCorrection(s1List[s1ptr].ParticleData)*LocalParticleWeight_s1/LocalTimeStep_s1/cellMeasure*CollisionLimitingFactor[1]; // calculate collision frequency taking into account the collision limiting factor
