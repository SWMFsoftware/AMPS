...
#pragma omp parallel for schedule(dynamic,nTotalCell) default (none) firstprivate (ParticleDataListLength,SigmaCrMax_nTest,SigmaCrMax_SafetyMargin) \
    ...
    firstprivate(PIC::MolecularData::MolecularModels::GetTotalCrossSection) 
#else
		  ...
		  meanWeightCorr[1]=sumWeightCorrection_s1/nParticleNumber[s1];
            ...
            while ((TimeCounter+=-log(rnd())/MajorFreq)<maxLocalTimeStep) {
              ...
              if (_PIC_INTERNAL_DEGREES_OF_FREEDOM_MODE_ == _PIC_MODE_ON_) {
                ...
                PIC::IDF::RedistributeEnergy(s0List[s0ptr].ParticleData,
                    s1List[s1ptr].ParticleData, cr, UpdateFlag, cell);
                for (int idim = 0; idim < 3; idim++) {
                  vrel[idim] = (cr > 1.0E-10) 
                ? vrel[idim] * cr / crInit 
                    : 0.0;
