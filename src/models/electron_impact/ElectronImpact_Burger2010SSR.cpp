...
double ElectronImpact::Burger2010SSR::GetTotalRateCoefficient(double *RateCoefficientTable,double ElectronTemperature,int nReactionChannels,double *log10RateCoefficientTable) {
  ...
  if (el<0.0) {
    ...
    for (nChannel=0;nChannel<nReactionChannels;nChannel++) {
      t=1.0E-6*pow(10.0,log10RateCoefficientTable[0+nChannel*log10RateCoefficientTableLength]);

      RateCoefficientTable[nChannel]=t;
      res+=t;
    }
  }
  else if (el>=log10RateCoefficientTableLength) {
    ...
    i1=i0+1,c1=1.0-c0;
    for (nChannel=0;nChannel<nReactionChannels;nChannel++) {
      t=1.0E-6*pow(10.0,c0*log10RateCoefficientTable[i0+nChannel*log10RateCoefficientTableLength]+c1*log10RateCoefficientTable[i1+nChannel*log10RateCoefficientTableLength]);
